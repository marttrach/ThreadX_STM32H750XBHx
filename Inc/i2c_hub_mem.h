...
static inline void *hub_spsc_alloc(hub_spsc_arena_t *a, uint32_t n)
{
    const uint32_t GUARD = HUB_DMA_ALIGN;
    n = HUB_ALIGN_UP(n);
    ...
    if (head >= tail) {
        ...
        DEBUG_DUMP(IOT_LOG_ERR, "[%s] alloc fail (wrap fail). n=%lu, free_end=%lu, tail=%lu, GUARD=%lu\r\n",
                   __arena_name(a), (unsigned long)n, (unsigned long)free_end,
                   (unsigned long)tail, (unsigned long)GUARD);
        return NULL;
        ...
        DEBUG_DUMP(IOT_LOG_ERR, "[%s] alloc fail. n=%lu, free_between=%lu, max_between=%lu, GUARD=%lu\r\n",
                   __arena_name(a), (unsigned long)n, (unsigned long)free_between,
                   (unsigned long)max_between, (unsigned long)GUARD);
        return NULL;
    }
}

static inline void *hub_spsc_alloc_ex(hub_spsc_arena_t *a, uint32_t n, uint32_t guard)
{
    n = HUB_ALIGN_UP(n);
...
static inline void *hub_sdram_alloc_rx(uint32_t n) { return hub_spsc_alloc_ex(&g_hub_rx_arena, n, 0U); }
static inline void *hub_sdram_alloc_tx(uint32_t n) { return hub_spsc_alloc_ex(&g_hub_tx_arena, n, HUB_DMA_ALIGN); }

// static inline void *hub_sdram_alloc_rx(uint32_t n) { return hub_spsc_alloc(&g_hub_rx_arena, n); }
static inline void  hub_sdram_free_rx (void *p, uint32_t n){ hub_spsc_free(&g_hub_rx_arena, p, n); }

// static inline void *hub_sdram_alloc_tx(uint32_t n) { return hub_spsc_alloc(&g_hub_tx_arena, n); }
