...
/**
 ******************************************************************************
 * @file    bsp_driver_esd.c for H7 (based on stm32h743i_eval_sd.c)
...
/**
  ...
  * @retval SD status
  */
__weak uint8_t BSP_eSD_Init(void)
{
  uint8_t sd_state = MSD_OK;
  /* Check if the SD card is plugged in the slot */
  if (BSP_eSD_IsDetected() != SD_PRESENT)
  {
    DEBUG_DUMP(IOT_LOG_DEBUG, "No SD Card Present\r\n");
...
/**
  ...
  * @retval Returns 0
  */
__weak uint8_t BSP_eSD_ITConfig(void)
{  
...
/**
  ...
  * @retval SD status
  */
__weak uint8_t BSP_eSD_ReadBlocks(uint32_t *pData, uint32_t ReadAddr, uint32_t NumOfBlocks, uint32_t Timeout)
{
  uint8_t sd_state = MSD_OK;
...
/**
  ...
  * @retval SD status
  */
__weak uint8_t BSP_eSD_WriteBlocks(uint32_t *pData, uint32_t WriteAddr, uint32_t NumOfBlocks, uint32_t Timeout)
{
  uint8_t sd_state = MSD_OK;
...
/**
  ...
  * @retval SD status
  */
__weak uint8_t BSP_eSD_ReadBlocks_DMA(uint32_t *pData, uint32_t ReadAddr, uint32_t NumOfBlocks)
{
  uint8_t sd_state = MSD_OK;
...
/**
  ...
  * @retval SD status
  */
__weak uint8_t BSP_eSD_WriteBlocks_DMA(uint32_t *pData, uint32_t WriteAddr, uint32_t NumOfBlocks)
{
  uint8_t sd_state = MSD_OK;
...
/**
  ...
  * @retval SD status
  */
__weak uint8_t BSP_eSD_Erase(uint32_t StartAddr, uint32_t EndAddr)
{
  uint8_t sd_state = MSD_OK;
...
/**
  ...
  * @retval None 
  */
void BSP_eSD_GetCardInfo(HAL_SD_CardInfoTypeDef *CardInfo)
{
...
uint8_t BSP_eSD_IsDetected(void)
{
  ...
  /* Normal(Not present) is High, present is Low, foxes, 2021/03/08  */
  status = HAL_GPIO_ReadPin(SDIO1_CD_GPIO_Port, SDIO1_CD_Pin) ? SD_NOT_PRESENT : SD_PRESENT;
